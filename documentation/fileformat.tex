...
struct Vertex
{
	...
	Kind of an index buffer. Three succeeding indices form a triangle. There may be invalid triangles with two or three equal indices. These are added for padding purposes of the hierarchy assignment.
	\begin{lstlisting}
struct Triangle
{
	uint32 indices[3];
	...
	% ************************************************************************ %
	\subsection{Material}
	There are two arrays for the material association: "materialref" and "materialassociation". "materialref" contains on entry for each material. The stored names reference material descriptors in an additional plain text file.
...
struct MaterialAssociation
{
	...
	The first bit (most significant bit) of \lstinline|firstChild| denotes if the child is a leaf. In case it is set the remaining 31 bit are a block index in the "triangles" buffer. One leaf can contain more than one triangle which is encoded in the \lstinline|elementSize| of the "triangles" header. Unused triangles are filled with invalid indices (all the same).
	\begin{lstlisting}
struct Node
{
	uint32 parent;
	...
	% ************************************************************************ %
	\subsection{[Bounding Geometry]}
	There are different types of bounding volumes for the hierarchy nodes: "bounding\_aabox", "bounding\_sphere". The number of elements in "hierarchy" and this array must be equal (This includes that "hierarchy" must exist).
	\begin{lstlisting}
struct Sphere
{
	Vec3 center;
