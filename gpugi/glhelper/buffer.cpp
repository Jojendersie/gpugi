...
namespace gl {
        ...
        m_sizeInBytes(_sizeInBytes)
    {
        ...
        GL_CALL(glNamedBufferStorage, m_bufferObject, _sizeInBytes, _data, GLenum(_usageFlags));
    ...
    void* Buffer::Map( std::uint32_t _offset, std::uint32_t _numBytes )
    {
        if( m_mapAccess == 0 )
            ...
            return glMapNamedBufferRange(m_bufferObject, _offset, _numBytes, m_mapAccess);
        return nullptr;
    }
