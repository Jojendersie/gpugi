...
private:
    ...
    void Initialize( const std::unique_ptr<uint32[]>* _sorted ) const;
    ...
    uint32 Build( const std::unique_ptr<uint32[]>* _sorted, uint32 _min, uint32 _max ) const;

	/// \brief Evaluate a cost function for the current node.
	/// \details The heuristic for a split is the sum from left and right
	///		sided node costs.
	float Costs( const FitMethod& _fit, uint32 _target, uint32 _parent, int _num, int _numOther ) const;

	/// \brief Compute bounding volume for all triangles in the range.
	void ComputeBoundingVolume( const std::unique_ptr<uint32[]>* _sorted, uint32 _min, uint32 _max, uint32 _target, const FitMethod& _fit ) const;
	...
	void Split( const std::unique_ptr<uint32[]>* _sorted, uint32 _min, uint32 _max, int _splitDimension, uint32 _splitIndex ) const;
};